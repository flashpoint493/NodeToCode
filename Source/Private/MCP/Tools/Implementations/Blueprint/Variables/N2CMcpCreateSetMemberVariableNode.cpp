...
#include "N2CMcpCreateSetMemberVariableNode.h"
...
// Register the tool
REGISTER_MCP_TOOL(FN2CMcpCreateSetMemberVariableNode)

FMcpToolDefinition FN2CMcpCreateSetMemberVariableNode::GetDefinition() const
{
    FMcpToolDefinition Definition(
        TEXT("create-set-member-variable-node"),
        TEXT("Creates a Set node for a member variable in the focused blueprint graph. This node can be used to assign values to member variables.")
    ,
    ...
    // Update defaultValue description
    TSharedPtr<FJsonObject> DefaultValueProp = SchemaProperties->GetObjectField(TEXT("defaultValue"));
    DefaultValueProp->SetStringField(TEXT("description"), TEXT("Optional default value to set on the input pin. This sets the pin's default value, not the variable's default value."));
...
FMcpToolCallResult FN2CMcpCreateSetMemberVariableNode::Execute(const TSharedPtr<FJsonObject>& Arguments)
{
    return ExecuteOnGameThread([this, Arguments]() -> FMcpToolCallResult
    {
        ...
        if (!ParseArguments(Arguments, VariableName, DefaultValue, Location, ParseError))
        {
        ...
        // Create the Set node
        UK2Node_VariableSet* SetNode = CreateSetNode(ActiveBlueprint, ActiveGraph, *Variable, Location);
        if (!SetNode)
        {
            return FMcpToolCallResult::CreateErrorResult(TEXT("Failed to create Set node"));
        }
        
        // Set default value if provided
        if (!DefaultValue.IsEmpty())
        {
            FString SetValueError;
            if (!SetNodeDefaultValue(SetNode, *Variable, DefaultValue, SetValueError))
            {
                FN2CLogger::Get().Log(
                    FString::Printf(TEXT("Warning: Could not set default value: %s"), *SetValueError),
        ...
        FNotificationInfo Info(FText::Format(
            NSLOCTEXT("NodeToCode", "SetNodeCreated", "Created Set node for variable '{0}'"),
        ...
        TSharedPtr<FJsonObject> Result = BuildSuccessResult(SetNode, *Variable, ActiveBlueprint, ActiveGraph);
...
bool FN2CMcpCreateSetMemberVariableNode::ParseArguments(
    ...
    FString& OutError)
{
    ...
    OutDefaultValue = ArgParser.GetOptionalString(TEXT("defaultValue"));
...
const FBPVariableDescription* FN2CMcpCreateSetMemberVariableNode::FindMemberVariable(
...
UK2Node_VariableSet* FN2CMcpCreateSetMemberVariableNode::CreateSetNode(
    UBlueprint* Blueprint,
    UEdGraph* Graph,
    const FBPVariableDescription& Variable,
    const FVector2D& Location)
{
    // Create a new K2Node_VariableSet
    ...
    FN2CLogger::Get().Log(
        FString::Printf(TEXT("Created Set node for variable '%s' at (%.2f, %.2f)"),
...
bool FN2CMcpCreateSetMemberVariableNode::SetNodeDefaultValue(
    ...
    FString& OutError)
{
    ...
    // Find the input value pin
    UEdGraphPin* ValuePin = SetNode->FindPin(Variable.VarName, EGPD_Input);
    if (!ValuePin)
    {
        // Try alternative pin name (sometimes it's the variable name with a space)
        ValuePin = SetNode->FindPin(FName(*(Variable.VarName.ToString() + TEXT(" "))), EGPD_Input);
    }
    
    if (!ValuePin)
    {
        OutError = TEXT("Could not find input value pin on Set node");
        return false;
    }
    
    // Set the default value directly on the pin
    const UEdGraphSchema* Schema = ValuePin->GetSchema();
    if (!Schema)
    {
        OutError = TEXT("No schema available for pin");
        return false;
    }
    
    // Use the schema to set the default value - this validates the value for the pin type
    const UEdGraphSchema_K2* K2Schema = Cast<UEdGraphSchema_K2>(Schema);
    if (!K2Schema)
    {
        OutError = TEXT("Not a K2 schema");
        return false;
    }
    
    // TrySetDefaultValue returns void, so we just call it and check if the value was set
    K2Schema->TrySetDefaultValue(*ValuePin, DefaultValue, false);
    
    // Check if the value was successfully set
    if (ValuePin->DefaultValue == DefaultValue || 
        ValuePin->DefaultTextValue.ToString() == DefaultValue ||
        ValuePin->DefaultObject != nullptr)
    {
        FN2CLogger::Get().Log(
            FString::Printf(TEXT("Set default value '%s' on input pin"), *DefaultValue),
            EN2CLogSeverity::Debug
        );
        return true;
    }
    ...
    {
        // The value might have been transformed or rejected
        FN2CLogger::Get().Log(
            FString::Printf(TEXT("Default value may have been adjusted from '%s' to '%s'"), 
                *DefaultValue, *ValuePin->DefaultValue),
            EN2CLogSeverity::Warning
        );
        return true; // Still consider it successful since the schema handled it
    }
}

TSharedPtr<FJsonObject> FN2CMcpCreateSetMemberVariableNode::BuildSuccessResult(
    UK2Node_VariableSet* SetNode,
    const FBPVariableDescription& Variable,
    UBlueprint* Blueprint,
    UEdGraph* Graph)
{
    ...
    static int32 SetNodeCounter = 0;
    FString NodeId = FString::Printf(TEXT("SetNode_%s_%d"), *Variable.VarName.ToString(), ++SetNodeCounter);
    ...
    Result->SetStringField(TEXT("nodeType"), TEXT("K2Node_VariableSet"));
    ...
    LocationObj->SetNumberField(TEXT("x"), SetNode->NodePosX);
    LocationObj->SetNumberField(TEXT("y"), SetNode->NodePosY);
    Result->SetObjectField(TEXT("location"), LocationObj);
    
    // Add pin information
    TArray<TSharedPtr<FJsonValue>> Pins = GetNodePins(SetNode);
    ...
    Result->SetStringField(TEXT("message"), FString::Printf(
        TEXT("Successfully created Set node for variable '%s' in graph '%s'"),
...
TArray<TSharedPtr<FJsonValue>> FN2CMcpCreateSetMemberVariableNode::GetNodePins(UK2Node_VariableSet* SetNode)
{
    ...
    for (UEdGraphPin* Pin : SetNode->Pins)
    {
        ...
        {
            PinInfo->SetStringField(TEXT("defaultValue"), Pin->DefaultValue);
        }
